...
battery = 100 #power multiplier based on max current delivery of battery, 100 = max, 50 = 1/2 power
try:
    while True:
 ...
 #       msg = s.recv(32)  # recieve up to 32 bytes of data
  #      msg = msg.decode("utf-8")  # decode the message
   #     if (msg[0:5] != "Axis:" and msg[0:7] != "Button:"):  # checking if string is messed up
    #        continue
     #   print(msg)
        msg = "             "
        # determining actuator tilt
        #flag = 0
        if (msg == "Button: 2"):  # left side down
            #flag = 1
            GPIO.output(FLIN1, 0)
            GPIO.output(FLIN2, 1)
            pwmFL.ChangeDutyCycle(60)
            sleep(1)
            #flag = 0
            GPIO.output(FRIN1, 0)
            GPIO.output(FRIN2, 0)
            ...
            continue
        elif (msg == "Button: 3"): # right side down
            #flag = 2
            GPIO.output(FRIN1, 0)
            GPIO.output(FRIN2, 1)
            pwmFR.ChangeDutyCycle(60)
            sleep(1)
            #flag = 0
            GPIO.output(FRIN1, 0)
            GPIO.output(FRIN2, 0)
            ...
            pwmFL.ChangeDutyCycle(30)
            continue
        elif (msg == "Button: 4"): # left side up
            #flag = 3
            GPIO.output(FLIN1, 1)
            GPIO.output(FLIN2, 0)
            pwmFL.ChangeDutyCycle(60)
            ...
            pwmFL.ChangeDutyCycle(30)
            continue
        elif (msg == "Button: 5"): # right side up 
            #flag = 4
            GPIO.output(FRIN1, 0)
            GPIO.output(FRIN2, 0)
            pwmFR.ChangeDutyCycle(30)
            ...
            pwmFL.ChangeDutyCycle(30)
            continue
        elif (msg[0:7] == "Button:"): # any other unneeded button presses
            continue
        #print("flag ============================              ==============")
        #print(flag)
 #       data = msg.split(" ")
     ...
     #   value = value[0:8]  # removing any extraneous text at end of value
      ...
      #  if(y_pos == 0):
       #     right_motors = -1 * x_pos
        ...
        #
        #if(flag == 0): # right up
        #    print("0                              00000000000000000000000000000000000")
        #    GPIO.output(FRIN1, 0)
        #    GPIO.output(FRIN2, 0)
        #    pwmFR.ChangeDutyCycle(30)
        #    GPIO.output(FLIN1, 0)
        ...
        if(1):
            #print("HERE")
            mux_read(0,0,0,0)
            CO = "54.00" #str(chan1.voltage)
            mux_read(1,0,0,0)
            CO2 = "43.00" # str(chan1.voltage)
            mux_read(0,1,0,0)
            LPG = "41.11" #str(chan1.voltage)
            mux_read(1,1,0,0)
            AM = "12.21" #str(chan1.voltage)
            mux_read(0,0,1,0)
            H2 = "21.21" #str(chan1.voltage)
            mux_read(1,0,1,0)
            MTH = "44.44" #str(chan1.voltage)
            mux_read(0,1,1,0)
            SMK = "11.12" #str(chan1.voltage)
            mux_read(0,0,0,0)
            ext_temp = "33.33" #str(1/((1/298.15) + (1/3380) * math.log(32767/chan2.value)-1))
            ...
            #elif(GPIO.input(Flame) == 0):
            #    flame = 'No Fire Present'
            Latitude = '32.730535'
            Longitude = '97.108131'
            top_string = "CO Level = " + CO +  " | CO2 Level = " + CO2 + " | Ammonium Sulfide Level = " + AM + " | Hydrogen Gas Level = " + H2 + " | Methane Level = " + MTH + " | Smoke Level = " + SMK
            bottom_string = "Internal Temperature = " + int_temp + " | Humidity = " + humidity2 + " | External Temperature = " + ext_temp + " | Flame = " + flame + " | Latitude = " +  Latitude + " | Longitude = " + Longitude
            #print(top_string)
            #print(bottom_string)
            t = open("/var/www/top.txt", "w+") # opens file to write sensor data to
